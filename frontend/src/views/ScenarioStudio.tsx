...
import { World, ScenarioParsedOutput } from '../types';
...
export const ScenarioStudio: React.FC<ScenarioStudioProps> = ({
  ...
  const [prompt, setPrompt] = useState<string>('Close Central Academic Avenue for emergency maintenance');
  const [isParsing, setIsParsing] = useState<boolean>(false);
  const [isSimulating, setIsSimulating] = useState<boolean>(false);
  const [parsed, setParsed] = useState<ScenarioParsedOutput | null>(null);
          ...
          <div className="parsed-results-card">
            ...
            <div className="parsed-body">
              ...
              {parsed.assumptions.length > 0 && (
                <div className="item-row">
                  <span className="key">Modeling Assumptions:</span>
              ...
              {parsed.valid ? (
                <div className="execution-action-bar">
                  <button className="btn-success" onClick={handleSimulate} disabled={isSimulating}>
                    {isSimulating ? 'Simulating Branch...' : '🚀 Execute Counterfactual Branch Now'}
                  </button>
                </div>
              ) : (
